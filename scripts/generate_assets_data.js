...
if (!Object.keys(loaderTypes.image).length) {
  ...
  for (var i in loaderTypes.image) {
    var key = i.split('/').pop();
    shell.ShellString('\n  export class ' + toPascalCase(key) + ' {').toEnd(assetsClassFile);
    shell.ShellString('\n    static getName(): string { return \'' + key + '\'; }\n').toEnd(assetsClassFile);
  ...
  for (var i in loaderTypes.atlas) {
    ...
    for (var t in loaderTypes.atlas[i]) {
      ...
      if (jsonExtensions.indexOf(loaderTypes.atlas[i][t]) !== -1) {
        shell.ShellString('\n  enum ' + toPascalCase(key) + 'Frames {').toEnd(assetsClassFile);
    ...
    shell.ShellString('\n  export class ' + toPascalCase(key) + ' {').toEnd(assetsClassFile);
    shell.ShellString('\n    static getName(): string { return \'' + key + '\'; }').toEnd(assetsClassFile);
    ...
    shell.ShellString('\n\n    static Frames = ' + toPascalCase(key) + 'Frames;').toEnd(assetsClassFile);
    shell.ShellString('\n  }').toEnd(assetsClassFile);
  }
}
shell.ShellString('\n}\n\n').toEnd(assetsClassFile);
  ...
  for (var i in loaderTypes.audiosprite) {
    ...
    for (var t in loaderTypes.audiosprite[i]) {
      ...
      if (jsonExtensions.indexOf(loaderTypes.audiosprite[i][t]) !== -1) {
        shell.ShellString('\n  enum ' + toPascalCase(key) + 'Sprites {').toEnd(assetsClassFile);
    ...
    shell.ShellString('\n\n    static Sprites = ' + toPascalCase(key) + 'Sprites;').toEnd(assetsClassFile);
    shell.ShellString('\n  }').toEnd(assetsClassFile);
  }
}
shell.ShellString('\n}\n\n').toEnd(assetsClassFile);
