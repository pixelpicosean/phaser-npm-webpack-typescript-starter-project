...
import { GameWidth, GameHeight, MaxGameWidth, MaxGameHeight } from '../config';
  ...
  public static calculateScreenMetrics(defaultWidth: number, defaultHeight: number, orientation: ScreenOrientation = ScreenOrientation.LANDSCAPE, maxGameWidth?: number, maxGameHeight?: number): ScreenMetrics {
    ...
    if (!maxGameWidth || !maxGameHeight) {
      if (orientation === ScreenOrientation.LANDSCAPE) {
        this.screenMetrics.maxGameWidth = Math.round(this.screenMetrics.defaultGameWidth * (MaxGameWidth / GameWidth));
        this.screenMetrics.maxGameHeight = Math.round(this.screenMetrics.defaultGameHeight * (MaxGameHeight / GameHeight));
      } else {
        this.screenMetrics.maxGameWidth = Math.round(this.screenMetrics.defaultGameWidth * (MaxGameHeight / GameHeight));
        this.screenMetrics.maxGameHeight = Math.round(this.screenMetrics.defaultGameHeight * (MaxGameWidth / GameWidth));
      }
    } else {
    ...
    let defaultAspectRatio: number = ((orientation === ScreenOrientation.LANDSCAPE) ? (GameWidth / GameHeight) : (GameHeight / GameWidth));
